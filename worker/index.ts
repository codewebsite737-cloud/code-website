  ...
  async fetch(
    ...
    if (url.pathname === "/_vinext/image") {
      if (!env.IMAGES) {
        ...
        if (sourcePath?.startsWith("/") && env.ASSETS) {
          const original = await env.ASSETS.fetch(
            new Request(new URL(sourcePath, request.url)),
          );
          return withSecurityHeaders(request, original);
        }
        return withSecurityHeaders(
          request,
          new Response("Image optimization is unavailable.", { status: 503 }),
        );
      }
    ...
    const isStaticAsset =
      ...
      /\.(css|js|json|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|eot)$/i.test(
        url.pathname,
      );
